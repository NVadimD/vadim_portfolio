...
const projectsNames = Array.from(
    document.querySelectorAll(".project-list__name")
);
const projectsCards = Array.from(document.querySelectorAll(".main-part"));
const projectsMarks = Array.from(
    document.querySelectorAll(".navigation__element")
);
...
leftArrow.addEventListener("click", () => {
    let ind = projectsNames.findIndex((item) =>
...
const projectsArr = [
    "https://nvadimd.github.io/Calorie-Calculator/",
...
projectBox.addEventListener("touchstart", function (e) {
    startX = e.touches[0].clientX;
    startY = e.touches[0].clientY;
});

projectBox.addEventListener("touchend", function (e) {
    ...
    } else if (Math.abs(deltaX) > Math.abs(deltaY) && deltaX < -12) {
        let ind = projectsNames.findIndex((item) =>
            item.classList.contains("active")
        );
        if (ind === 0) {
